"""
Ecommerce Profit Dashboard v4.1
...
def cargar(data_bytes, name):
    ...
    if name.lower().endswith((".xlsx", ".xls")):
        ...
        for enc in ("utf-8", "latin-1", "cp1252"):
            try:
                buf.seek(0)
                df = pd.read_csv(buf, dtype=str, encoding=enc, sep=None, engine="python")
                break
            except Exception:
                continue
        else:
            buf.seek(0)
            df = pd.read_csv(buf, dtype=str, encoding="latin-1", on_bad_lines="skip")
    df.columns = df.columns.str.strip().str.upper()
    alias = {
        "TOTAL DE LA ORDEN": ["TOTAL DE LA ORDEN", "TOTAL_DE_LA_ORDEN"],
        "PRECIO PROVEEDOR": ["PRECIO PROVEEDOR", "PRECIO_PROVEEDOR"],
        "PRECIO FLETE": ["PRECIO FLETE", "PRECIO_FLETE"],
        "COSTO DEVOLUCION FLETE": ["COSTO DEVOLUCION FLETE", "COSTO_DEVOLUCION_FLETE"],
        "GANANCIA": ["GANANCIA", "PROFIT"],
        "CANTIDAD": ["CANTIDAD", "QTY"],
        "ESTATUS": ["ESTATUS", "STATUS", "ESTADO"],
        "PRODUCTO": ["PRODUCTO", "PRODUCT"],
        "FECHA": ["FECHA", "DATE"],
        "CIUDAD DESTINO": ["CIUDAD DESTINO", "CIUDAD"],
    }
    for canon, vs in alias.items():
        for v in vs:
            if v in df.columns and canon not in df.columns:
                ...
                break
    num_cols = ["TOTAL DE LA ORDEN", "PRECIO PROVEEDOR", "PRECIO PROVEEDOR X CANTIDAD",
                "PRECIO FLETE", "COSTO DEVOLUCION FLETE", "GANANCIA", "CANTIDAD", "COMISION"]
    for col in num_cols:
...
def filtrar_fecha(df, ini, fin):
    if "FECHA" not in df.columns:
        return df
    return df[
        (df["FECHA"] >= pd.Timestamp(ini)) &
        (df["FECHA"] <= pd.Timestamp(fin) + pd.Timedelta(days=1) - pd.Timedelta(seconds=1))
    ].copy()


def detect_country(df):
    cities_co = {"BOGOTA", "BOGOTÁ", "MEDELLIN", "MEDELLÍN", "CALI", "BARRANQUILLA", "CARTAGENA",
                 "BUCARAMANGA", "PEREIRA", "CUCUTA", "CÚCUTA", "MANIZALES", "IBAGUE", "IBAGUÉ",
                 "PASTO", "SANTA MARTA", "VILLAVICENCIO", "NEIVA", "MONTERIA", "MONTERÍA",
                 "POPAYAN", "POPAYÁN", "ARMENIA", "SINCELEJO", "VALLEDUPAR", "TUNJA", "DOSQUEBRADAS"}
    cities_ec = {"QUITO", "GUAYAQUIL", "CUENCA", "AMBATO", "PORTOVIEJO", "MACHALA", "DURÁN",
                 "LOJA", "MANTA", "SANTO DOMINGO", "RIOBAMBA", "ESMERALDAS", "IBARRA", "LATACUNGA"}
    cities_gt = {"GUATEMALA", "MIXCO", "VILLA NUEVA", "QUETZALTENANGO", "ESCUINTLA", "CHINAUTLA",
                 "AMATITLAN", "HUEHUETENANGO", "COBAN", "COBÁN", "CHIMALTENANGO", "ANTIGUA",
                 "JALAPA", "JUTIAPA", "RETALHULEU", "MAZATENANGO", "ZACAPA"}
    if "CIUDAD DESTINO" in df.columns:
        ciudades = set(df["CIUDAD DESTINO"].dropna().str.upper().str.strip().unique())
        co = len(ciudades & cities_co)
        ec = len(ciudades & cities_ec)
        gt = len(ciudades & cities_gt)
        if co > ec and co > gt:
            return "Colombia"
        if ec > co and ec > gt:
            return "Ecuador"
...
def extraer_base_producto(nombre):
    ...
    words = [w for w in n.split()
             if not re.match(r'^\d+$', w) and w not in ("X", "DE", "EL", "LA", "EN", "CON", "PARA", "POR")]
...
def fb_spend_acct(tok, cid, i, f):
    if not tok or not cid:
        return 0.0
    cid = cid.strip()
    if not cid.startswith("act_"):
        cid = f"act_{cid}"
    try:
        r = requests.get(f"https://graph.facebook.com/v21.0/{cid}/insights",
                         params={"access_token": tok,
                                 "time_range": json.dumps({"since": i, "until": f}),
                                 "fields": "spend", "level": "account"}, timeout=30)
        d = r.json()
        if "data" in d and d["data"]:
            return float(d["data"][0].get("spend", 0))
    except Exception:
        pass
    return 0.0
    ...
    try:
        r = requests.get(f"https://graph.facebook.com/v21.0/{cid}/insights",
                         params={"access_token": tok,
                                 ...
                                 "level": "campaign", "limit": 500}, timeout=30)
        d = r.json()
        if "data" in d:
            return pd.DataFrame([{"campaign_name": x.get("campaign_name", ""),
                                   "spend": float(x.get("spend", 0))} for x in d["data"]])
    except Exception:
        pass
    return pd.DataFrame(columns=["campaign_name", "spend"])
...
def fb_list_accts(tok):
    ...
    try:
        r = requests.get("https://graph.facebook.com/v21.0/me/adaccounts",
                         params={"access_token": tok, "fields": "name,account_id,currency", "limit": 50},
                         timeout=15)
        d = r.json()
        if "data" in d:
            return [{"id": a.get("account_id", ""), "name": a.get("name", ""),
                      "cur": a.get("currency", "")} for a in d["data"]]
    except Exception:
        pass
    return []
...
def tt_spend_acct(tok, aid, i, f):
    ...
    try:
        r = requests.post("https://business-api.tiktok.com/open_api/v1.3/report/integrated/get/",
                          ...
                          json={"advertiser_id": aid, "report_type": "BASIC",
                                "data_level": "AUCTION_ADVERTISER",
                                "dimensions": ["advertiser_id"], "metrics": ["spend"],
                                "start_date": i, "end_date": f}, timeout=30)
        d = r.json()
        if d.get("code") == 0 and d.get("data", {}).get("list"):
            return float(d["data"]["list"][0]["metrics"].get("spend", 0))
    except Exception:
        pass
    return 0.0
...
def status_pill(s):
    s = s.upper()
    if "ENTREGADO" in s:
        return '<span class="pill p-ent">✅ Entregado</span>'
    if "CANCELADO" in s:
        return '<span class="pill p-can">⊘ Cancelado</span>'
    if "DEVOLUCION" in s or "DEVOLUCIÓN" in s:
        return '<span class="pill p-dev">↩ Devolución</span>'
    if "TRÁNSITO" in s or "TRANSITO" in s:
        return '<span class="pill p-tra">🚚 En Tránsito</span>'
    if "DESPACHADA" in s or "GUIA" in s:
        return '<span class="pill p-env">✈ Enviado</span>'
    if "NOVEDAD" in s:
        return '<span class="pill p-nov">⚠ Novedad</span>'
    if "REPARTO" in s or "RUTA" in s or "BODEGA" in s:
        return '<span class="pill p-tra">📦 En Camino</span>'
    return '<span class="pill p-pen">⏳ Pendiente</span>'


def aggregate_orders(df):
    F = {c: c in df.columns for c in ["ID", "ESTATUS", "TOTAL DE LA ORDEN", "PRODUCTO",
         "CANTIDAD", "GANANCIA", "PRECIO FLETE", "PRECIO PROVEEDOR", "COSTO DEVOLUCION FLETE",
         "CIUDAD DESTINO", "FECHA", "GRUPO_PRODUCTO", "PRECIO PROVEEDOR X CANTIDAD"]}
    if not F["ID"]:
        return df, F
    ag = {}
    for c, fn in [("TOTAL DE LA ORDEN", "first"), ("ESTATUS", "first"), ("PRECIO FLETE", "first"),
                   ("COSTO DEVOLUCION FLETE", "first"), ("GANANCIA", "sum"), ("FECHA", "first"),
                   ("CIUDAD DESTINO", "first"), ("PRODUCTO", "first"), ("CANTIDAD", "sum"),
                   ("PRECIO PROVEEDOR", "first"), ("PRECIO PROVEEDOR X CANTIDAD", "sum"),
                   ("GRUPO_PRODUCTO", "first")]:
        if c in df.columns:
            ag[c] = fn
    df_ord = df.groupby("ID").agg(ag).reset_index() if ag else df.copy()
    return df_ord, F
...
def calc_kpis(df_ord, F, g_fb, g_tt):
    ...
    n_desp = len(df_nc)
    aov = fact_bruto / n_ord if n_ord > 0 else 0
    g_ads = g_fb + g_tt
    roas = fact_neto / g_ads if g_ads > 0 else 0
    ...
    n_tra = sum(v for k, v in sc.items() if any(x in k for x in
                ["TRANSITO", "TRÁNSITO", "DESPACHADA", "REPARTO", "RUTA", "BODEGA", "GUIA", "ENVIADO"]))
    ...
    cpr = 0
    if not de.empty and "PRECIO PROVEEDOR X CANTIDAD" in de.columns:
        cpr = de["PRECIO PROVEEDOR X CANTIDAD"].sum()
    elif not de.empty and F.get("PRECIO PROVEEDOR") and F.get("CANTIDAD"):
        cpr = (de["PRECIO PROVEEDOR"] * de["CANTIDAD"]).sum()

    dd = df_ord[df_ord["ESTATUS"].isin(["DEVOLUCION", "DEVOLUCIÓN"])].copy() if F.get("ESTATUS") else pd.DataFrame()
    fl_dev = 0
    if not dd.empty:
        if F.get("PRECIO FLETE") and F.get("COSTO DEVOLUCION FLETE"):
            fl_dev = dd[["PRECIO FLETE", "COSTO DEVOLUCION FLETE"]].max(axis=1).sum()
        elif F.get("PRECIO FLETE"):
            fl_dev = dd["PRECIO FLETE"].sum()
    ...
    return dict(n_ord=n_ord, fact_bruto=fact_bruto, fact_neto=fact_neto, n_desp=n_desp, aov=aov,
                ...
                pct_can=pct_can, pct_ent=pct_ent, tasa_ent=pct_ent,
...
with st.sidebar:
    ...
    if usar_api:
        ...
        if fb_accts:
            ...
            if v:
                save_cfg("fb_cids", v)
                fb_cids = [x.strip() for x in v.split(",") if x.strip()]
        st.markdown("###### 🎵 TikTok")
        ...
        if tt_token:
            save_cfg("tt_token", tt_token)
        if tt_input:
            save_cfg("tt_aids", tt_input)
            tt_aids = [x.strip() for x in tt_input.split(",") if x.strip()]
        st.caption("⚠️ FB reporta en COP. Se distribuye por país.")
    ...
    with dc1:
        f_ini = st.date_input("Desde", value=hoy - timedelta(days=30))
    with dc2:
        f_fin = st.date_input("Hasta", value=hoy)

    # Manual ads per country (only if not using API)
    ads_manual = {}
    if not usar_api:
        st.divider()
        st.markdown("##### 💵 Ads Manual por País")
        for pn in PAISES:
            ads_manual[pn] = st.number_input(f"Gasto {pn}", min_value=0.0, value=0.0,
                                              step=1000.0, key=f"gm_{pn}")
...
st.markdown(f'<p style="text-align:center;font-size:.88rem;color:#64748B;margin-bottom:1.5rem">{f_ini.strftime("%d/%m/%Y")} – {f_fin.strftime("%d/%m/%Y")}</p>', unsafe_allow_html=True)

# Check if any country has data
...
if not any_data:
    ...
    st.markdown('<p style="color:#94A3B8;font-size:.85rem;margin-bottom:1rem">Arrastra o haz clic para subir. Detectamos el país automáticamente, o selecciona uno específico.</p>', unsafe_allow_html=True)

    # Main area: auto-detect upload
    st.markdown("#### 🔍 Subir y auto-detectar país")
    f_auto = st.file_uploader("Arrastra tu archivo aquí o haz clic para seleccionar",
                               type=["csv", "xlsx", "xls"], key="up_auto_main",
    ...
    if f_auto:
        ...
        if detected:
            ...
            st.success(f"✅ Detectado: {PAISES[detected]['flag']} {detected} — {len(temp_df):,} filas")
            st.rerun()
        else:
            st.warning("No se pudo detectar el país. Selecciona manualmente:")

    # Main area: per-country uploads
    st.markdown("#### 📁 O sube por país")
    cols_up = st.columns(len(PAISES))
    for i, (pn, pi) in enumerate(PAISES.items()):
        with cols_up[i]:
            f = st.file_uploader(f"{pi['flag']} {pn}", type=["csv", "xlsx", "xls"],
                                  key=f"up_main_{pn}")
            if f:
                st.session_state[f"_b_{pn}"] = f.getvalue()
                st.session_state[f"_n_{pn}"] = f.name
                st.rerun()

    st.stop()

# ── Also show small uploaders at sidebar bottom for when data exists ──
with st.sidebar:
    st.divider()
    ...
    for pn, pi in PAISES.items():
        ...
        if st.session_state.get(f"_b_{pn}"):
            st.caption(f"✅ {st.session_state[f'_n_{pn}']}")
...
if usar_api:
    for c in fb_cids:
        all_camps = pd.concat([all_camps, fb_camps_acct(fb_token, c, si, sf)])

# Compute total ads once
if usar_api:
    gfb_total = sum(fb_spend_acct(fb_token, c, si, sf) for c in fb_cids)
    gtt_total = sum(tt_spend_acct(tt_token, a, si, sf) for a in tt_aids)
else:
    gfb_total = sum(ads_manual.values())
    gtt_total = 0
...
for pn in PAISES:
    kb = f"_b_{pn}"
    kn = f"_n_{pn}"
    if st.session_state.get(kb) is None:
        continue
...
if not country_data:
    st.warning("Sin datos en el rango de fechas seleccionado.")
    st.stop()


# ═══════════════════════ PRODUCT MAPPING UI ═══════════════════════
with st.expander("📦 Agrupación de Productos (se guarda permanentemente)", expanded=False):
    for pn, cd in country_data.items():
        if "PRODUCTO" not in cd["df"].columns:
            continue
        ...
        for gn, members in saved.items():
            for m in members:
                rows.append({"Producto Original": m, "Grupo": gn})
        if not rows:
        ...
        edited_pg = st.data_editor(df_pg, use_container_width=True, hide_index=True,
                                    key=f"pg_{pn}", num_rows="dynamic")
        if st.button(f"💾 Guardar {pn}", key=f"save_pg_{pn}"):
            new_groups = defaultdict(list)
            for _, r in edited_pg.iterrows():
                o = str(r.get("Producto Original", "")).strip()
                g = str(r.get("Grupo", "")).strip()
                if o and g:
                    new_groups[g].append(o)
            persistent_maps[pn] = dict(new_groups)
            save_mappings(persistent_maps)
            st.success(f"✅ Guardado para {pn}")
            st.rerun()
    ...
    with st.expander("🔗 Mapeo Campañas FB → Grupo Producto", expanded=False):
        ...
        if "mapeo" not in st.session_state:
            ...
            for p in pfb:
                m = p
                for g in grp_list:
                    if p.upper() in g.upper() or g.upper() in p.upper():
                        m = g
                        break
                rows.append({"Campaña Facebook": p, "Grupo Producto": m})
            if not rows:
                rows = [{"Campaña Facebook": g, "Grupo Producto": g} for g in grp_list]
            st.session_state["mapeo"] = pd.DataFrame(rows)

        me = st.data_editor(st.session_state["mapeo"], num_rows="dynamic",
                             use_container_width=True, key="me")
        if st.button("🔄 Actualizar Mapeo", key="btn_mapeo"):
            st.session_state["mapeo"] = me
            st.rerun()

    md = {}
    if "mapeo" in st.session_state:
        for _, r in st.session_state["mapeo"].iterrows():
            a = str(r.get("Campaña Facebook", "")).strip().upper()
            b = str(r.get("Grupo Producto", "")).strip().upper()
            if a and b:
                md[a] = b
    if md:
        all_camps["pf"] = all_camps["campaign_name"].apply(lambda x: extraer_prod_camp(x).upper())
        all_camps["pd"] = all_camps["pf"].map(md).fillna(all_camps["pf"])
        gxp = all_camps.groupby("pd")["spend"].sum().to_dict()


# ═══════════════════════ TABS ═══════════════════════
tab_names = ["🏠 Dashboard Global"]
for pn in country_data:
    tab_names.append(f"{PAISES[pn]['flag']} {pn}")
...
with tabs[0]:
    st.markdown('<p class="section-hdr">Resumen por País</p>', unsafe_allow_html=True)
    for pn, cd in country_data.items():
        k = cd["kpis"]
        pi = PAISES[pn]
        ...
        <div class="row2 r4" style="margin-bottom:.5rem">
            ...
            <div class="kcard red"><div class="icon r">🎯</div><div class="lbl">GASTO ADS</div><div class="val lg red">{fmt(k['g_ads'],pn)}</div><div class="sub">FB + TT</div></div>
    ...
    if len(country_data) > 1:
        st.divider()
        comp = [{"País": f"{PAISES[pn]['flag']} {pn}", "Facturado": cd["kpis"]["fact_neto"],
                 "Ads": cd["kpis"]["g_ads"], "Utilidad": cd["kpis"]["u_real"]}
                for pn, cd in country_data.items()]
    ...
    <div class="lgrid">
        <div class="litem" style="border-color:rgba(16,185,129,0.3)"><p>✅ ENTREGADO</p><h3 style="color:{C['profit']}">{t_ent:,}</h3></div>
        <div class="litem" style="border-color:rgba(239,68,68,0.3)"><p>❌ CANCELADO</p><h3 style="color:{C['loss']}">{t_can:,}</h3></div>
        <div class="litem" style="border-color:rgba(59,130,246,0.3)"><p>🚚 TRÁNSITO</p><h3 style="color:{C['blue']}">{t_tra:,}</h3></div>
        <div class="litem" style="border-color:rgba(245,158,11,0.3)"><p>↩️ DEVOLUCIÓN</p><h3 style="color:{C['warn']}">{t_dev:,}</h3></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with tabs[idx + 1]:
        ...
        with ct1:
            ...
            if F.get("FECHA") and F.get("TOTAL DE LA ORDEN"):
                daily = df_ord.groupby(df_ord["FECHA"].dt.date).agg(
                    Fac=("TOTAL DE LA ORDEN", "sum")).reset_index().rename(columns={"FECHA": "Fecha"})
                gc1, gc2 = st.columns(2)
                with gc1:
                    fig = go.Figure()
                    fig.add_trace(go.Scatter(x=daily["Fecha"], y=daily["Fac"], mode="lines",
                                             fill="tozeroy", fillcolor="rgba(16,185,129,0.1)",
                                             line=dict(color=C["profit"], width=2),
                                             hovertemplate="%{y:,.0f}<extra></extra>"))
                    fig.update_layout(**pl(title="FACTURACIÓN NETA"))
                    st.plotly_chart(fig, use_container_width=True, key=f"ch_fac_{pn}")
                with gc2:
                    if F.get("ESTATUS"):
                        edf = df_ord["ESTATUS"].value_counts().reset_index()
                        edf.columns = ["E", "N"]
                        cm = {}
                        for s in edf["E"]:
                            if "ENTREGADO" in s: cm[s] = C["profit"]
                            elif "CANCELADO" in s: cm[s] = C["loss"]
                            elif "DEVOLUCION" in s or "DEVOLUCIÓN" in s: cm[s] = C["orange"]
                            elif "TRANSITO" in s or "DESPACHADA" in s: cm[s] = C["blue"]
                            elif "NOVEDAD" in s: cm[s] = C["warn"]
                            else: cm[s] = C["muted"]
                        ...
                        f3.update_traces(textinfo="percent",
                                          hovertemplate="%{label}: %{value}<extra></extra>")
                        st.plotly_chart(f3, use_container_width=True, key=f"ch_pie_{pn}")
        ...
        with ct2:
            ...
            if not gcol:
                st.warning("No hay columna de productos.")
            else:
                productos = sorted(df_f[gcol].dropna().unique())
                ixo = k["fact_neto"] / k["n_desp"] if k["n_desp"] > 0 else 0
                sc1, sc2 = st.columns(2)
                with sc1:
                    dg = st.slider("% Entrega", 50, 100, 80, key=f"dg_{pn}")
                with sc2:
                    rb = st.number_input("Colchón Dev.", value=1.40, min_value=1.0,
                                          max_value=3.0, step=0.05, key=f"rb_{pn}")

                pe_key = f"pe_{pn}"
                if pe_key not in st.session_state:
                    st.session_state[pe_key] = {}
                for p in productos:
                    if p not in st.session_state[pe_key]:
                        st.session_state[pe_key][p] = float(dg)

                prows = []
                for prod in productos:
                    dp = df_f[df_f[gcol] == prod]
                    dpnc = dp[dp["ESTATUS"] != "CANCELADO"] if F.get("ESTATUS") else dp
                    ...
                    cp = ue * pp
                    fl = oe * fp + odv * fp * rb
                    ap = gxp.get(prod.upper(), 0)
                    ip = oe * ixo
                    ut = ip - cp - fl - ap
                    prows.append({"Producto": prod, "Órdenes": int(od), "% Entrega": pe,
                                  "Ingreso": round(ip), "Costo": round(cp), "Fletes": round(fl),
                                  "Ads": round(ap), "Utilidad": round(ut)})
                ...
                for _, r in dfp.iterrows():
                    ...
                    <div style="margin-bottom:1.2rem">
                        ...
                        <div style="display:flex;gap:8px;align-items:center">
                            <div style="flex:1;background:#111827;border-radius:4px;height:22px;position:relative;overflow:hidden">
                                <div style="width:{ip_pct:.0f}%;height:100%;background:rgba(16,185,129,0.2);border-radius:4px"></div>
                                <div style="position:absolute;top:0;left:0;width:{ut_pct:.0f}%;height:100%;background:{C['profit']};border-radius:4px;opacity:.7"></div>
                            </div>
                            <span style="font-family:'JetBrains Mono';font-size:.82rem;color:{ut_col};width:130px;text-align:right;font-weight:600">{fmt(r['Utilidad'],pn)}</span>
                        </div>
                        <div style="display:flex;gap:16px;margin-top:3px;font-size:.7rem;color:{C['sub']}">
                            <span>Ingreso: {fmt(r['Ingreso'],pn)}</span>
                            <span>Costo: -{fmt(r['Costo'],pn)}</span>
                            <span>Fletes: -{fmt(r['Fletes'],pn)}</span>
                            <span>Ads: -{fmt(r['Ads'],pn)}</span>
                        </div>
                    </div>"""
                ...
                with st.expander("📝 Editar % Entrega por producto"):
                    ed = st.data_editor(dfp,
                        column_config={"% Entrega": st.column_config.NumberColumn(
                            "% ENTREGA", min_value=0, max_value=100, step=1, format="%d")},
                        disabled=[c for c in dfp.columns if c != "% Entrega"],
                        use_container_width=True, hide_index=True, key=f"pt_{pn}")
                    if ed is not None:
                        for _, r in ed.iterrows():
                            st.session_state[pe_key][r["Producto"]] = r["% Entrega"]
        ...
        with ct3:
            ...
            st.markdown(f"""
            <div class="kcard {ucls}" style="padding:2rem;margin-bottom:1.5rem">
                <div class="icon {'g' if k['u_real']>=0 else 'r'}" style="width:48px;height:48px;font-size:1.3rem">💰</div>
                <div class="lbl">UTILIDAD REAL (LA VERDAD FINANCIERA)</div>
                <div class="val xl {ucls}">{fmt(k['u_real'],pn)}</div>
                <div class="sub">Margen: {mg:.0f}%</div>
            </div>
            <div class="row2 r3">
                <div class="kcard green"><div class="icon g">✅</div><div class="lbl">INGRESO (ENTREGADOS)</div><div class="val md green">{fmt(k['ing_real'],pn)}</div><div class="sub">Solo estado ENTREGADO</div></div>
            ...
            <div class="row2 r2">
                <div class="kcard red"><div class="icon r">⚠️</div><div class="lbl">FLETES DEVOLUCIONES</div><div class="val md red">-{fmt(k['fl_dev'],pn)}</div><div class="sub">Mayor entre flete y costo devolución</div></div>
                <div class="kcard"><div class="icon b">🚚</div><div class="lbl">FLETES EN TRÁNSITO</div><div class="val md white">-{fmt(k['fl_tra'],pn)}</div><div class="sub">Riesgo en camino</div></div>
            </div>
            """, unsafe_allow_html=True)

            # Cascade
            st.markdown('<p class="section-hdr">Cascada Financiera</p>', unsafe_allow_html=True)
            items = [("Ingreso Entregados", k["ing_real"], True), ("Costo Producto", k["cpr"], False),
                     ("Gasto Ads", k["g_ads"], False), ("Fletes Dev.", k["fl_dev"], False),
                     ("Fletes Tránsito", k["fl_tra"], False)]
            mx_c = max(k["ing_real"], 1)
            ch = ""
            for lb, vl, pos in items:
                bp = min(vl / mx_c * 100, 100) if mx_c > 0 else 0
                bc = C["profit"] if pos else C["loss"]
                sg = "" if pos else "-"
            ...
            up = min(abs(k["u_real"]) / mx_c * 100, 100) if mx_c > 0 else 0
            uc = C["profit"] if k["u_real"] >= 0 else C["loss"]
            us = "" if k["u_real"] >= 0 else "-"
            st.markdown(f"""
            <div class="kcard" style="padding:1rem 1.5rem">
                {ch}
                <div style="border-top:2px solid #1E293B;margin:8px 0"></div>
                <div class="cas-row" style="border:none">
                    <div class="cas-lbl" style="font-weight:700;color:#F1F5F9">UTILIDAD REAL</div>
                    <div class="cas-bar-wrap"><div class="cas-bar" style="width:{up:.0f}%;background:{uc}"></div></div>
                    <div class="cas-amt" style="color:{uc};font-size:.95rem">{us}{fmt(abs(k['u_real']),pn)}</div>
                </div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with ct4:
            ...
            cols = [c for c in ["ID", "FECHA", "PRODUCTO", "CANTIDAD", "TOTAL DE LA ORDEN",
                                "PRECIO FLETE", "CIUDAD DESTINO", "ESTATUS"] if c in df_ord.columns]
            if cols:
                dfo = df_ord[cols].copy()
                if "FECHA" in dfo.columns:
                    dfo = dfo.sort_values("FECHA", ascending=False)
                nr = len(dfo)
                hmap = {"ID": "ID ORDEN", "FECHA": "FECHA", "PRODUCTO": "PRODUCTO",
                        "CANTIDAD": "CANT.", "TOTAL DE LA ORDEN": "TOTAL",
                        "PRECIO FLETE": "FLETE", "CIUDAD DESTINO": "CIUDAD", "ESTATUS": "ESTADO"}
                hdrs = "".join(f"<th>{hmap.get(c,c)}</th>" for c in cols)
                ps = 50
                opk = f"op_{pn}"
                if opk not in st.session_state:
                    st.session_state[opk] = 0
                tp = max(1, (nr - 1) // ps + 1)
                s = st.session_state[opk] * ps
                e = min(s + ps, nr)
                rh = ""
                for _, row in dfo.iloc[s:e].iterrows():
                    ...
                    for c in cols:
                        v = row[c]
                        if c == "ID":
                            tds += f'<td class="id">ORD-{v}</td>'
                        elif c == "FECHA":
                            tds += f'<td>{v.strftime("%d %b %Y") if pd.notna(v) else "-"}</td>'
                        elif c in ("TOTAL DE LA ORDEN", "PRECIO FLETE"):
                            tds += f'<td class="mono">{fmt(v, pn)}</td>'
                        elif c == "CANTIDAD":
                            tds += f'<td style="text-align:center">{int(v)}</td>'
                        elif c == "ESTATUS":
                            tds += f'<td>{status_pill(str(v))}</td>'
                        elif c == "CIUDAD DESTINO":
                            tds += f'<td>{str(v).title() if pd.notna(v) else "-"}</td>'
                        else:
                            tds += f'<td>{v}</td>'
                    rh += f"<tr>{tds}</tr>"
                ...
                with pc1:
                    if st.button("← Anterior", disabled=st.session_state[opk] == 0, key=f"pv_{pn}"):
                        st.session_state[opk] -= 1
                        st.rerun()
                with pc2:
                    st.markdown(f'<p style="text-align:center;color:#64748B;font-size:.85rem">Pág {st.session_state[opk]+1}/{tp}</p>', unsafe_allow_html=True)
                with pc3:
                    if st.button("Siguiente →", disabled=st.session_state[opk] >= tp - 1, key=f"nx_{pn}"):
                        st.session_state[opk] += 1
                        st.rerun()
...
with tabs[-1]:
    ...
    total_ads_all = total_fb + total_tt
    total_ord_pub = sum(cd["kpis"]["n_ord"] for cd in country_data.values())
    total_ent_pub = sum(cd["kpis"]["n_ent"] for cd in country_data.values())
    cpa_ord = total_ads_all / total_ord_pub if total_ord_pub > 0 else 0
    cpa_ent = total_ads_all / total_ent_pub if total_ent_pub > 0 else 0

    st.markdown(f"""
    <div class="row2 r4">
        <div class="kcard purple"><div class="icon p">📢</div><div class="lbl">GASTO TOTAL ADS</div><div class="val lg purple">{fmt(total_ads_all,'Colombia')}</div><div class="sub">Todos los países (COP)</div></div>
        ...
        <div class="kcard red"><div class="icon r">🎯</div><div class="lbl">CPA POR ORDEN</div><div class="val lg red">{fmt(cpa_ord,'Colombia')}</div><div class="sub">CPA Entregado: {fmt(cpa_ent,'Colombia')}</div></div>
    </div>
    """, unsafe_allow_html=True)

    # By country
    if len(country_data) > 1:
        ads_data = [{"País": f"{PAISES[pn]['flag']} {pn}",
                     "Facebook": cd["kpis"]["g_fb"], "TikTok": cd["kpis"]["g_tt"]}
                    for pn, cd in country_data.items()]
    ...
    if gxp:
        prod_ads = pd.DataFrame([{"Producto": k, "Gasto": v}
                                  for k, v in sorted(gxp.items(), key=lambda x: -x[1])])
        ...
        for pn, cd in country_data.items():
            ...
            if gcol not in cd["df_ord"].columns:
                continue
            for prod in cd["df_ord"][gcol].dropna().unique():
                dp = cd["df_ord"][cd["df_ord"][gcol] == prod]
                n = len(dp)
                ad = gxp.get(prod.upper(), 0)
                if n > 0 and ad > 0:
                    cpa_rows.append({"Producto": prod, "CPA": round(ad / n), "Órdenes": n, "Gasto": ad})
        if cpa_rows:
            dfcpa = pd.DataFrame(cpa_rows)
            fcpa = px.bar(dfcpa, x="Producto", y="CPA", color_discrete_sequence=[C["orange"]], text="CPA")
            fcpa.update_layout(**pl(title="COSTO POR ADQUISICIÓN (CPA) POR PRODUCTO"))
            fcpa.update_traces(texttemplate="%{y:,.0f}", textposition="outside",
                                hovertemplate="CPA: %{y:,.0f}<extra></extra>")
    ...
    for pn, cd in country_data.items():
        if "FECHA" in cd["df_ord"].columns and cd["kpis"]["g_ads"] > 0:
            daily_o = cd["df_ord"].groupby(cd["df_ord"]["FECHA"].dt.date).size().reset_index(name="Ordenes")
            daily_o.columns = ["Fecha", "Ordenes"]
            avg_d = cd["kpis"]["g_ads"] / max(len(daily_o), 1)
            daily_o["CPA"] = avg_d / daily_o["Ordenes"].replace(0, 1)
            ft = go.Figure()
            ft.add_trace(go.Bar(x=daily_o["Fecha"], y=daily_o["Ordenes"], name="Órdenes",
                                 marker_color=C["profit"], hovertemplate="%{y}<extra></extra>"))
            ft.add_trace(go.Scatter(x=daily_o["Fecha"], y=daily_o["CPA"], name="CPA diario",
                                     yaxis="y2", line=dict(color=C["orange"], width=2),
                                     hovertemplate="%{y:,.0f}<extra></extra>"))
...
st.markdown(f'<div style="text-align:center;color:#475569;font-size:.75rem;padding:.5rem 0">Profit Dashboard v4.1 · {loaded} países · {f_ini.strftime("%d/%m")} – {f_fin.strftime("%d/%m/%Y")}</div>', unsafe_allow_html=True)
